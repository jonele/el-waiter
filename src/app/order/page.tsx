...
import { pushToGunther, guntherUrlFromBridge } from "@/lib/guntherPrint";
...
function OrderPageInner() {
  ...
  async function sendToKitchen() {
    ...
    try {
      ...
      // 3b. Gunther print — LAN-first (same machine as Bridge, port 8089), Supabase fallback
      if (supabase) {
        const gAlias = cp?.receipt_printer_name ?? null;
        const gLanUrl = guntherUrlFromBridge(settings.bridgeUrl);
        void pushToGunther(supabase, vid, newOrder.id, activeTable.name, waiter.name, orderItems, gAlias, newOrder.created_at, gLanUrl);
      }
