...
export async function GET(req: NextRequest) {
  ...
  const r = await fetch(
    `${RSRV_BASE}/api/floor-manager/waitlist?venue_id=${encodeURIComponent(venueId)}&status=active`,
...
export async function POST(req: NextRequest) {
  ...
  if (!body.venue_id || !body.party_name || !body.party_size) {
  ...
  const r = await fetch(`${RSRV_BASE}/api/floor-manager/waitlist`, {
    method: "POST",
